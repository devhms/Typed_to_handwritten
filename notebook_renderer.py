...
try:
    ...
    def generate_fiber_map(w, h):
        return np.zeros((h, w), dtype=np.uint8)

    class ThixotropicPBI:
        def render_bio_stroke(self, *args):
            pass

        def apply_capillary_wicking(self, *args):
            pass
...
_STYLE_PROFILE_DEFAULTS = {
    "messy": {
        "variation_magnitude": 0.06,
        "drift_intensity": 3.5,
        "margin_panic_strength": 0.4,
        "fatigue_slant_rate": 0.05,
    },
    "neat": {
        "variation_magnitude": 0.015,
        "drift_intensity": 0.6,
        "margin_panic_strength": 0.1,
        "fatigue_slant_rate": 0.01,
    },
}

_NOTEBOOK_DEFAULTS = NotebookConfig()


def apply_style_profile_defaults(cfg: NotebookConfig) -> None:
    """
    Apply style profile defaults only for fields still at global defaults.

    This preserves explicit overrides from callers (for example, UI sliders
    or benchmark scripts) while still providing style-specific behavior.
    """
    profile = _STYLE_PROFILE_DEFAULTS.get(cfg.style)
    if not profile:
        return

    for field_name, profile_value in profile.items():
        current_value = getattr(cfg, field_name)
        default_value = getattr(_NOTEBOOK_DEFAULTS, field_name)
        if current_value == default_value:
            setattr(cfg, field_name, profile_value)


def _parse_plain_text_blocks(body_text: str) -> List[dict]:
    blocks: List[dict] = []
...
def _coerce_document_blocks(
    document_blocks=None,
    *,
    body_text: Optional[str] = None,
    title: Optional[str] = None,
) -> List[dict]:
    if document_blocks is not None:
        if not isinstance(document_blocks, list):
            raise TypeError("document_blocks must be a list of block dictionaries")
        return document_blocks

    content = body_text or ""
    blocks = _parse_plain_text_blocks(content)
    title_text = (title or "").strip()
    if title_text:
        return [
            {"type": "header", "content": title_text, "level": 1},
            {"type": "spacer", "content": ""},
        ] + blocks
    return blocks
...
class HandwritingRenderer:
    ...
    def _apply_unified_pbi(
        self,
        masterpiece_canvas: np.ndarray,
        mask: np.ndarray,
        offset_x: int,
        offset_y: int,
        ...
        self.pbi_engine.render_bio_stroke(
            np.array(points), masterpiece_canvas, groove_canvas, fiber_map
        )
    ...
    def get_line_wander(self, n_chars: int) -> np.ndarray:
        return generate_line_baseline_wander(
            n_chars, font_size_px=self.cfg.body_font_size, bias=self.cfg.bias, rng=self._np_rng
        )
    ...
    def _sample_ink_color(self) -> Tuple[int, int, int, int]:
        ...
        dr, dg = sample_bivariate_gaussian(
            sd1=self._bc.scale_sd(3.0), sd2=self._bc.scale_sd(2.0), ro=0.6, rng=self._np_rng
        )
    ...
    def render_char(
        ...
        fast_path_ok = (
            abs(rotation) < 0.1 and abs(scale - 1.0) < 0.01 and abs(dx) < 0.5 and abs(dy) < 0.5
        )
        if fast_path_ok:
            ...
            tmp, final_px, final_py = perturb_glyph_mask(
                ...
                variation_magnitude=self.cfg.variation_magnitude,
                rng=self._np_rng,
            )
            ...
            if masterpiece_canvas is not None:
                ...
                self._apply_unified_pbi(
                    masterpiece_canvas, mask_alpha, paste_x, paste_y, groove_canvas, fiber_map
                )
                ...
                if overlay_alpha > 0.0:
                    if overlay_alpha < 1.0:
                        ...
                        rgba[:, :, 3] = (rgba[:, :, 3].astype(np.float32) * overlay_alpha).astype(
                            np.uint8
                        )
    ...
    def wrap_text(self, text, font, max_w):
        ...
        for w in words:
            ...
            if current_w + ww + (sw if current else 0) > max_w and current:
                ...
                current.append(w)
                current_w += ww + (sw if len(current) > 1 else 0)
        if current:
            lines.append(" ".join(current))
        return lines
...
class HeaderRenderer:
    ...
    def render(self, canvas, lines, title, y):
        f, tf = (
            self.hw._load_font(self.cfg.header_font_size),
            self.hw._load_font(self.cfg.title_font_size),
        )
        ...
        pts = [
            (px, y + 8 + int(self.hw._np_rng.integers(-1, 2)))
            for px in range(tx - 5, tx + round(tf.getlength(title)) + 5, 6)
        ]
        if len(pts) >= 2:
            ImageDraw.Draw(canvas).line(pts, fill=ink, width=2)
...
def _snap_to_ruled_line(y, cfg):
    ...
    return (
        cfg.first_line_y + math.ceil((y - cfg.first_line_y) / cfg.line_spacing) * cfg.line_spacing
    )
...
def render_notebook_page(
    document_blocks=None,
    output_path="output/rendered_page.png",
    seed=42,
    ...
    *,
    ...
    """
    cfg = replace(config) if config is not None else NotebookConfig()
    document_blocks = _coerce_document_blocks(
    ...
    if masterpiece:
        masterpiece_canvas = np.zeros(
            (cfg.page_h, cfg.page_w, 4), dtype=np.uint8
        )  # Particle ink layer
        groove_canvas = np.zeros(
            (cfg.page_h, cfg.page_w), dtype=np.uint8
        )  # Paper crush (groove) layer
        fiber_map = generate_fiber_map(cfg.page_w, cfg.page_h)  # Substrate fiber distribution

    apply_style_profile_defaults(cfg)
    ...
    header_font = hw._load_font(
        cfg.header_font_size
    )  # Use IndieFlower/Kalam for headers if configured
    ...
    for block in document_blocks:
        ...
        for i, line in enumerate(wrapped_lines):
            ...
            hw._mixture = make_default_mixture(
                bias=cfg.bias + (line_count * cfg.fatigue_slant_rate), rng=hw._np_rng
            )
    ...
    if masterpiece and masterpiece_canvas is not None:
        if fiber_map is None:
            fiber_map = np.zeros((cfg.page_h, cfg.page_w), dtype=np.uint8)
        ...
        for c in range(3):
            img_bgr[:, :, c] = (
                alpha * masterpiece_canvas[:, :, c] + (1.0 - alpha) * img_bgr[:, :, c]
            ).astype(np.uint8)
...
def _paginate_body_text(body_text: str, max_chars_per_page: int) -> List[str]:
    ...
    for token in tokens:
        token_cost = len(token) + 1
        if current_tokens and current_chars + token_cost > max_chars_per_page:
...
def render_notebook_multi_page(
    ...
    seed: int = 42,
    config: Optional[NotebookConfig] = None,
    masterpiece: bool = False,
    max_chars_per_page: int = 1800,
) -> List[str]:
    pages_text = _paginate_body_text(body_text, max_chars_per_page=max(200, max_chars_per_page))
    ...
    for idx, page_text in enumerate(pages_text):
        page_title = None
        if title:
            page_title = title if idx == 0 else f"{title} (cont.)"
        page_path = output_path / f"page_{idx + 1:02d}.png"
        rendered = render_notebook_page(
        ...
        rendered_pages.append(str(rendered))
...
if __name__ == "__main__":
    render_notebook_page(
        [
            {"type": "header", "content": "Stability Test v8.5", "level": 1},
            {"type": "paragraph", "content": "Perfect synchronization achieved."},
        ],
        output_path="output/test_v85_sync.png",
    )
