...
def simulate_transcription(text: str,
    ...
    while i < len(chars):
        ...
        # ── Correct keystroke ─────────────────────────────────────────────
        event_type = 'correction_retype' if (
            session.events and session.events[-1].event_type == 'backspace'
        ) else 'press'
    ...
    simulated_rate = (
        session.total_corrections / max(session.total_chars_typed, 1) * 100
    )
